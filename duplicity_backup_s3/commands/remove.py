...
@click.option(
    "--older-than",
    "time",
    help="Delete all backup sets older than given time. eg. '8h', '7D', '1M', 'now', '2019-06-03'. Old backup sets "
    "will not be deleted if backup sets newer than time depend on them. Use `--force` to actually delete them.",
)
@click.option(
    "--all-but-n-full",
    type=int,
    help="Delete all backups sets that are older than the <COUNT> last full backup (in other words, "
    "keep the last count full backups and associated incremental sets). <COUNT> must be larger than zero. "
    "A value of 1 means that only the single most recent backup chain will be kept. "
...
@click.option(
    ...
    "--all-inc-but-n-full",
    type=int,
    help="Delete *incremental* sets of all backups sets that are older than the <COUNT> last full backup "
    "(in other words, keep only old full backups and not their increments). <COUNT> must be larger than zero. "
